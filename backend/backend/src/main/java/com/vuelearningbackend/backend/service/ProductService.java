...
import com.vuelearningbackend.backend.repository.ImagesRepository;
...
//        }
        Category category = Optional.ofNullable(
                categoryRepository.findByName(addProductRequest.getName())
        ).orElseGet(() -> {
            if (addProductRequest.getCategory() != null) {
                Category category1 = new Category(addProductRequest.getCategory().getName());
